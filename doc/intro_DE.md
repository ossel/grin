...
* **Überprüfung von Nullsummen.** Die Summe der Outputs minus die Summer der Inputs muss immer gleich Null sein. Dies beweist, dass die Transaktion kein neues Geld aus dem Nichts ersschaffen hat, ohne dabei die tatsächlichen Beträge der Transaktion zu offenbaren.
...
Problematisch dabei ist lediglich, dass es nur eine endliche Anzahl benutzbarer v-Werte gibt und man diese der Reihe nach durchprobieren könnte um den Transaktionsbetrag zu erraten. Außerdem deckt die Kenntnis eines v1-Wertes (von einer vorherigen Transaktion) und des entsprechenden `v1*H`-Werts alle restlichen v1-Werte der Outputs der gesamten Blockchain auf. Aus diesem Grund führen wir eine weitere elliptische Kurve _G_ und einen privaten Schlüssel _r_, der als sogenannter *blinding factor* fungiert, ein. (Orginal: practically _G_ is just another generator point on the same curve group as _H_)
